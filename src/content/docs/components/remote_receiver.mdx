...
- **on_hob2hood** (*Optional*, [Automation](/automations)): An automation to perform when a
  Hob2Hood remote code code has been decoded. A variable `x` of type <APIStruct text="remote_base::Hob2HoodData" path="remote_base::Hob2HoodData" />
  is passed to the automation for use in lambdas.
...
- **hob2hood**: Trigger on a decoded Hob2Hood remote code with the given data.
  ...
  - **command** (**Required**, enum): The Hob2Hood command to trigger on, see dumper output for more info.
    - ``light_off``
    - ``light_on``
    - ``fan_off``
    - ``fan_low``
